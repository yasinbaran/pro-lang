...
Set-ExecutionPolicy Unrestricted -Scope CurrentUser -Confirm:$false -Force
$CurUserRomaingPath=$env:APPDATA
$CurUserLocalAppData=$Env:USERPROFILE + "\AppData\Local"
...
if(Test-Path "$CurUserRomaingPath\Microsoft\Communicator"){
    Remove-Item -Recurse -Force $CurUserRomaingPath\Microsoft\Communicator
}

if(Test-Path "$CurUserRomaingPath\Microsoft\Office\15.0\Lync"){
    Remove-Item -Recurse -Force $CurUserRomaingPath\Microsoft\Office\15.0\Lync
}
if(Test-Path "$CurUserRomaingPath\Microsoft\Office\14.0\Lync"){
    Remove-Item -Recurse -Force $CurUserRomaingPath\Microsoft\Office\14.0\Lync
}
if(Test-Path "$CurUserLocalAppData\Microsoft\Communicator"){
    Remove-Item -Recurse -Force $CurUserLocalAppData\Microsoft\Communicator
}

if(Test-Path "$CurUserLocalAppData\Microsoft\Office\15.0\Lync"){
    Remove-Item -Recurse -Force $CurUserLocalAppData\Microsoft\Office\15.0\Lync
}
if(Test-Path "$CurUserLocalAppData\Microsoft\Office\14.0\Lync"){
    Remove-Item -Recurse -Force $CurUserLocalAppData\Microsoft\Office\14.0\Lync
}
...
foreach ($Entry in $Result)
        {
            if ($Entry)
            {
                ...
                if ($Line.Contains('Target: '))
                {
                    Write-Verbose $Line
                    $Target = $Line.Replace('Target: ',''); 
                    Remove-CachedCredential $Target
                    }
                }
        }
